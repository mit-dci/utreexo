...
func (t *TxIn) SerializeSize() int {
	...
	return 40 + util.VarIntSerializeSize(uint64(len(t.SignatureScript))) +
...
func (t TxWitness) SerializeSize() int {
	...
	n := util.VarIntSerializeSize(uint64(len(t)))
	...
	for _, witItem := range t {
		n += util.VarIntSerializeSize(uint64(len(witItem)))
		n += len(witItem)
	}
...
func (t *TxOut) SerializeSize() int {
	...
	// PkScript bytes.
	return 8 + util.VarIntSerializeSize(uint64(len(t.PkScript))) + len(t.PkScript)
}
...
func (msg *MsgTx) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if flag[0] != 0 && enc == WitnessEncoding {
		for _, txin := range msg.TxIn {
			...
			witCount, err := util.ReadVarInt(r, pver)
			if err != nil {
				returnScriptBuffers()
				return err
			}
...
func (msg *MsgTx) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	err = util.WriteVarInt(w, pver, count)
	if err != nil {
		return err
	}
...
func (msg *MsgTx) baseSize() int {
	...
	n := 8 + util.VarIntSerializeSize(uint64(len(msg.TxIn))) +
		util.VarIntSerializeSize(uint64(len(msg.TxOut)))
...
func (msg *MsgTx) PkScriptLocs() []int {
	...
	// input.
	n := 4 + util.VarIntSerializeSize(uint64(len(msg.TxIn))) +
		util.VarIntSerializeSize(uint64(numTxOut))
	...
	for i, txOut := range msg.TxOut {
		...
		// PkScript.
		n += 8 + util.VarIntSerializeSize(uint64(len(txOut.PkScript)))
		pkScriptLocs[i] = n
...
func writeTxWitness(w io.Writer, pver uint32, version int32, wit [][]byte) error {
	err := util.WriteVarInt(w, pver, uint64(len(wit)))
	if err != nil {
		return err
	}
