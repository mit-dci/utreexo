...
func TestCalcDirtyNodes(t *testing.T) {
	...
	for i, test := range tests {
		dirtyRows := calcDirtyNodes(test.moves, test.numLeaves, test.forestRows)

		fmt.Println("dirty Nodes", dirtyRows, i)

		if len(dirtyRows) != len(test.expected) {
			t.Fatalf("TestCalcDirtyNodes fail: expected %d rows, got %d",
				len(test.expected), len(dirtyRows))
		}

		for row, dirtyRow := range dirtyRows {
			if len(dirtyRow) != len(test.expected[row]) {
				t.Fatalf("TestCalcDirtyNodes fail: at row %d, expected %d nodes but got %d",
					row, len(test.expected[row]), len(dirtyRow))
			}
